...
@section('page_title', 'Crear Categoría de Productos')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-lg font-bold text-slate-800">Nueva Categoría</h2>
            <p class="text-xs text-slate-500">Agrupe los platos del menú (Hamburguesas, Bebidas, etc.).</p>
        </div>
    ...
    <form action="{{ route('categorias.store') }}" method="POST" class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
        @csrf
        ...
        <div class="space-y-4 text-xs">
            <div>
                ...
                <input type="text" name="nombre" id="nombre" value="{{ old('nombre') }}" required placeholder="Ej: Hamburguesas Especiales" class="w-full rounded-lg border border-slate-300 p-2.5 focus:ring-2 focus:ring-orange-500 focus:outline-none">
            </div>

            <div>
                <label for="descripcion" class="block font-semibold text-slate-700 mb-1">Descripción</label>
                <textarea name="descripcion" id="descripcion" rows="3" placeholder="Breve descripción del grupo de productos..." class="w-full rounded-lg border border-slate-300 p-2.5 focus:ring-2 focus:ring-orange-500 focus:outline-none">{{ old('descripcion') }}</textarea>
            </div>
